...
adds(daraula(Sala,Professor),[at(Professor,uff),deuaula(Professor),horario(M),desligado(Sala,X)]
	,[at(Professor,uff),deuaula(Professor),horario(M),fechado(Sala),desligado(Sala,X)],sala):-
	equipamento(X),
	usa(Professor, X),
	aula(Professor,Sala,_,M).
	
deletes(daraula(Sala,Professor),[at(Professor,Sala),ligado(Sala,X),horario(N)],sala) :-	
	equipamento(X),
	usa(Professor, X),
	aula(Professor,Sala,N,_).
